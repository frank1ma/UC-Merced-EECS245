...
int main(void) {
	...
	int NUM_MAX_THREADS = 12;//omp_get_max_threads();     // number of threads used
	clock_t t1,t2;
	...
	t1 = clock();
	// Step 1  Parallel
#pragma omp parallel for num_threads(NUM_MAX_THREADS) schedule(static,CHUNK_SIZE)
		for (i = 0; i < NUM_ELEMENTS; i++) {
			int lid, j,gid;
...
#pragma omp parallel for num_threads(NUM_MAX_THREADS) schedule(static,CHUNK_SIZE)
	...
	// Step 4 check values of prefix sum
		...
		cout << "Static Schdedule : total time is " << (t2 - t1) * 1.0 / CLOCKS_PER_SEC * 1000 << " ms " << endl;
